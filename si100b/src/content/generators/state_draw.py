...
def draw_d14(name, data):
    ...
    conf.value_formatter = lambda x:"{:,}".format(int(x)) if x != None else None
...
def draw_general(data):
    ...
    chart.render_to_file(f"{savepath}/state_general.svg")
...
def draw_vac(data):
    ...
    conf.truncate_label = -1
    conf.value_formatter = lambda x:"{:,}".format(int(x)) if x != None else None
    conf.y_labels = ymark(data['d1_vac'])
    conf.legend_box_size = 63
    conf.show_legend = True
    conf.legend_at_bottom = True
    conf.margin = 0
    chart = pygal.HorizontalBar(conf)
    chart.add("Fully Vaccined Pop", data['full_vac'])
    chart.add("Partially Vaccined Pop", data['d1_vac'])
    chart.render_to_file(f"{savepath}/state_vac.svg")
...
def draw_ins(data):
    ...
    chart.render_to_file(f"{savepath}/state_ins.svg")
...
def draw_pop(data, kind):
    ...
    for key, value in data.items():
        chart.add(key, [value])
    chart.render_to_file(f"{savepath}/state_pop{kind}.svg")