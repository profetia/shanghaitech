...
configue = {
    "cases":  
        {
            "color": "#c8435c", 
            "title": "Cumulative Cases in 90 Days", 
    ...
    "deaths": 
        {
            "color": "#5b69ab", 
            "title": "Cumulative Deaths in 90 Days", 
...
def get_data(data):
    ...
    new_death = []
    length = len(data)
    for i in range(length - 91, length):
        date.append(data.loc[i, 'date'][5:])
        new_case.append(data.loc[i, 'cases'] - data.loc[length - 92, 'cases'])
        new_death.append(data.loc[i, 'deaths'] - data.loc[length - 92, 'deaths'])
...
def draw(name, data):
    ...
    conf.x_labels_major = [data['date'][i] for i in np.linspace(2, 88, 10, dtype = int)]
...
def d90_statics(raw_data):
    data = deepcopy(raw_data)
    data = get_data(data)
    draw("cases", data)
    draw("deaths", data)
    return {
        "d90cases": "{:,}".format(data['cases'][-1]), 
        "d90deaths": "{:,}".format(data["deaths"][-1])
        }